...
export function studioColor(id: string | null | undefined): string {
...
export function studioLoc(s: Studio | null | undefined): string {
  if (!s) return '';
  return STUDIO_CITY[s.id] || '';
}

export function studioName(s: Studio | null | undefined): string {
  if (!s) return '';
