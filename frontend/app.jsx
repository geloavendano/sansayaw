// sa'nsayaw — MNL dance class aggregator
// Production frontend. Fetches live schedule from Supabase.

const { useState, useMemo, useEffect, useRef } = React;

// ── CONFIG ───────────────────────────────────────────────────
const SUPABASE_URL      = "https://ljebzcgfydaknyekwlqv.supabase.co";
const SUPABASE_ANON_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImxqZWJ6Y2dmeWRha255ZWt3bHF2Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3Nzk2MzMyNjUsImV4cCI6MjA5NTIwOTI2NX0.rXqFC1e-G5ojDdJQKCF5RgH1QwYQJ1Fn0t1mG_URZxo";

// InstaPay QR — set to image URL (hosted file or base64 data URI) when ready.
// e.g. const INSTAPAY_QR_URL = "https://yoursite.com/qr.png";
...
const T = {
  ...
  secondary:    "#ff6da3",
  tertiary:     "#f5b15c",
  headingFont:  "'Geist', 'Helvetica Neue', sans-serif",
  bodyFont:     "'Geist', 'Helvetica Neue', sans-serif",
...
function App() {
  ...
  useEffect(() => {
    loadAppData().then(setData).catch(e => setErr(e?.message || "Could not load schedule"));
  }, []);

  if (err)   return <ErrorScreen msg={err} />;
  if (!data) return <LoadingScreen />;
  return <DanceApp {...data} />;
}

function LoadingScreen() {
  return (
    <div style={{
      width:"100%", height:"100svh", background:T.bg, fontFamily:T.bodyFont,
      display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:18,
      backgroundImage:`radial-gradient(120% 50% at 80% 0%, ${hexA(T.accent,0.08)} 0%, transparent 60%)`,
    }}>
      <Wordmark />
      <div style={{ color:T.textDim, fontSize:13 }}>Loading schedule…</div>
    </div>
  );
}
...
function ErrorScreen({ msg }) {
  return (
    <div style={{
      width:"100%", height:"100svh", padding:32,
      background:T.bg, color:T.text, fontFamily:T.bodyFont,
      display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center",
      gap:10, textAlign:"center",
    }}>
      <Wordmark />
      <div style={{ marginTop:20, fontSize:15, fontWeight:500 }}>Could not load schedule</div>
      <div style={{ fontSize:12.5, color:T.textDim, maxWidth:320, lineHeight:1.65 }}>{msg}</div>
    </div>
  );
}
...
// DANCE APP
// ─────────────────────────────────────────────────────────────
function DanceApp({ studios, instrs, classes, lastUpdated }) {
  ...
  return (
    <div ref={ref} style={{ display:"flex", gap:6, overflowX:"auto", padding:"0 16px 14px", scrollbarWidth:"none" }}>
      {days.map((d,i) => {
        ...
        // Visual hierarchy: selected > today > other
              ...
              <div style={{
                ...
                <div style={{ textAlign:"center", padding:"0 24px" }}>
                  ...
                  <div style={{ fontSize:10.5, color:T.textMute, marginTop:4, lineHeight:1.5 }}>
                    Set INSTAPAY_QR_URL in app.jsx
                  </div>
                </div>
              </div>
            )}
